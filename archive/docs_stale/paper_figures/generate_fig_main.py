...
nn_d32  = [0.037, 0.020, 0.019]  # d=32 model (N=128 still training)

ax2.plot(Ns_g, sc_g, 'b-o', label='SC', zorder=3)
ax2.plot(Ns_g, nn_g, 'r-s', label='NN-SC ($d{=}16$)', zorder=3)
ax2.plot(Ns_g[:3], nn_d32, '--^', color='tab:red', label='NN-SC ($d{=}32$)', zorder=4, markersize=7)
...
# Annotate d=32 beating SC
ax2.annotate('$d{=}32$ beats SC', xy=(64, 0.020), xytext=(45, 0.005),
             arrowprops=dict(arrowstyle='->', color='tab:red', lw=1), fontsize=7, color='tab:red')
