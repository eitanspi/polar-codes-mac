#!/usr/bin/env python3
"""
eval_all_n256_checkpoints.py — Quick single-model BLER for all N=256
checkpoints in saved_models/ that share the SimpleMLP_Gmac signature.

Writes results/crc_scl_expansion/gmac_N256_all_checkpoints.json
"""
...
def eval_bler(model, channel, Au, Av, fu, fv, b, n_cw=1000, batch=25):
    model.eval()
    errs = 0; total = 0
...
def main():
    ...
    candidates = [
        ...
        'n256_long_best.pt',
        'n256_long_latest.pt',
        'n256_emb_noise_best.pt',
        'n256_self_play_best.pt',
        'n256_swap5_best.pt',
        'n256_tree_transfer_best.pt',
        'scratch_n256_1500.pt',
        'multilevel_N256_best.pt',
        'hybrid_n256_latest.pt',
        'fastce_n256_best.pt',
        'ncg_pure_neural_N256.pt',  # probably BEMAC (3-vocab) — will fail to load
    ]

    print(f"\n{'='*78}")
    print(f"  All N={N} GMAC Class B checkpoints (1000 cw each)")
    ...
    for name in candidates:
        ...
        t0 = time.time()
        try:
            bler = eval_bler(m, channel, Au, Av, fu, fv, b, 1000)
            dt = time.time() - t0
            print(f"  {name:40s}  BLER={bler:.4f}  [{dt:.0f}s]")
            results[name] = {'bler': bler, 'n_cw': 1000, 'time_s': round(dt, 1)}
        except Exception as e:
            print(f"  [run-fail] {name}: {e}")
            results[name] = f'run_err: {e}'

    out = os.path.join(BASE, 'results', 'crc_scl_expansion',
                        'gmac_N256_all_checkpoints.json')
