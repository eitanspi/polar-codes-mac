#!/usr/bin/env python3
"""eval_all_n128_checkpoints.py — check if better N=128 GMAC B checkpoint exists."""
...
def eval_bler(model, channel, Au, Av, fu, fv, b, n_cw=2000, batch=50):
    model.eval()
    errs = 0; total = 0
...
def main():
    ...
    b = make_path(N, N // 2)
    candidates = [
        'ncg_gmac_mlp_N128.pt',
        'ncg_gmac_d32_N128.pt',
        'ncg_gmac_freeze_extend_N128.pt',
        'ncg_gmac_levelenc_N128.pt',
        'ncg_gmac_perlevel_N128.pt',
        'd32_30hr_N128_best.pt',
    ]
    print(f"\n  All N={N} GMAC Class B checkpoints (2000 cw each)", flush=True)
    results = {}
    for name in candidates:
        ...
        if isinstance(m, str):
            print(f"  [fail] {name}: {m[:50]}"); results[name] = m; continue
        t0 = time.time()
        try:
            bler = eval_bler(m, channel, Au, Av, fu, fv, b, 2000, batch=50)
            dt = time.time() - t0
            print(f"  {name:35s}  BLER={bler:.4f}  [{dt:.0f}s]", flush=True)
            results[name] = {'bler': bler, 'n_cw': 2000, 'time_s': round(dt, 1)}
        except Exception as e:
            print(f"  [run-fail] {name}: {e}"); results[name] = f'run_err: {e}'
    out = os.path.join(BASE, 'results', 'crc_scl_expansion', 'gmac_N128_all_checkpoints.json')
