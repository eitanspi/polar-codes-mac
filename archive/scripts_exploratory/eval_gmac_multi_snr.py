#!/usr/bin/env python3
"""
eval_gmac_multi_snr.py — GMAC Multi-SNR evaluation for paper waterfall curves.

Evaluate NN-SC and SC at SNR = 3, 4, 5, 6, 7, 8 dB for N=64 and N=128.
Tests generalization of models trained at 6dB to other SNR values.
"""
...
N_CW = 3000  # per (N, SNR) point


def load_design(N, ku, kv, snr_db):
    """Load GMAC MC design for specific SNR."""
    n = int(math.log2(N))
    dp = os.path.join(BASE, 'designs', f'gmac_B_n{n}_snr{snr_db}dB.npz')
    if not os.path.exists(dp):
        print(f"  WARNING: Design file {dp} not found, trying GA design")
...
def load_gmac_model(N, device='cpu'):
    """Load GMAC neural decoder (SimpleMLP_Gmac wrapper)."""
    ...
    # Handle key mismatch (z_enc -> z_encoder)
    fixed = {}
    for k, v in sd.items():
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"  GMAC Multi-SNR Evaluation — Class B")
    ...
    for N in [64, 128]:
        ...
        for snr_db in SNR_VALUES:
            ...
            Au, Av, fu, fv = load_design(N, ku, kv, snr_db)
            ...
            N_results[str(snr_db)] = {
                ...
                'sc_bler': bler_sc, 'nn_sc_bler': bler_nn,
                'ratio': round(ratio, 3),
                'n_cw': N_CW,
            }
    ...
    out_path = os.path.join(out_dir, 'gmac_multi_snr_evaluation.json')
    ...
    # Print summary table
    print(f"\n{'='*80}")
    print(f"  SUMMARY: NN-SC / SC BLER Ratio by SNR")
    print(f"{'='*80}")
    print(f"  {'SNR':>5s}", end='')
    for N in [64, 128]:
        print(f"  {'N='+str(N)+' SC':>10s}  {'N='+str(N)+' NN':>10s}  {'ratio':>6s}", end='')
    print()
    for snr_db in SNR_VALUES:
