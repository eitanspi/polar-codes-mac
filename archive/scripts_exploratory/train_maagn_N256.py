...
train_maagn_N256.py
===================
Train chained NPD for MA-AGN MAC at N=256 with d=16 h=100 BiGRU on CPU.
Warm-starts from N=128 checkpoint if available.
...
WARMSTART_S1 = os.path.join(RESULTS_DIR, 'maagn_d16_h100_s1_N128_best.pt')
WARMSTART_S2 = os.path.join(RESULTS_DIR, 'maagn_d16_h100_s2_N128_best.pt')
...
def eval_chained(model, channel, Au, Av, fu_set, fv_set, n_cw=5000, batch=16, seed=777):
    n = int(math.log2(N))
    br = bit_reversal_perm(n)
    br_t = torch.from_numpy(br.copy()).long()
    model.stage1.eval()
...
def main():
    ...
    print(f'sigma2={SIGMA2:.6f}, alpha={ALPHA}')
    print(f'ku={KU}, kv={KV}, batch={BATCH}')
    print(f'iters_s1={ITERS_S1}, iters_s2={ITERS_S2}')
    print(f'{"="*60}')
    ...
    # Warm-start from N=128
    try_warmstart(model, 1)

    n = int(math.log2(N))
    br = bit_reversal_perm(n)
    br_t = torch.from_numpy(br.copy()).long()
    log_file = os.path.join(RESULTS_DIR, f'maagn_d{D}_h{HIDDEN}_N{N}.log')
    tag_s1 = f'maagn_d{D}_h{HIDDEN}_s1_N{N}'
    tag_s2 = f'maagn_d{D}_h{HIDDEN}_s2_N{N}'
