#!/usr/bin/env python3
"""eval_crc_ensemble_n512.py — CRC-aided 3-model ensemble at N=512.
Mirrors the N=256 script with adjusted rates and checkpoint set."""
...
N = 512; KU = KV = 246
SIGMA2 = 10 ** (-6.0 / 10)
MODELS = ['n512_long_latest.pt', 'n512_long_best.pt', 'campaign_n512_best.pt']
FALLBACK = 'n512_long_latest.pt'
...
def main():
    ...
    print(f"\n  CRC-aided 3-model ensemble at N={N}  ku={KU}  kv={KV}", flush=True)
    ...
    with torch.no_grad():
        while total < n_cw:
            ...
            if total % 100 == 0:
                msg = f"  {total}/{n_cw}  "
                for name in model_names:
                    short = name.replace('.pt','').replace('_best','').replace('_latest','_lat').replace('campaign_','camp_').replace('n512_','')[:10]
    ...
    out = os.path.join(BASE, 'results', 'crc_scl_expansion', 'gmac_N512_crc_ensemble.json')
