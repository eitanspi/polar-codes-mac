"""
run_design_abnmac.py
====================
MC genie design script for ABN-MAC, all three code classes (A, B, C).

Usage:
    python scripts/run_design_abnmac.py --class C --N 1024 --hours 1
    python scripts/run_design_abnmac.py --class B --N 1024 --hours 0.5
    python scripts/run_design_abnmac.py --class A --N 1024 --hours 2
...
# ════════════════════════════════════════════════════════════════════
#  CODE CLASS CONFIGURATIONS (ABN-MAC)
# ════════════════════════════════════════════════════════════════════

CLASS_CONFIG = {
    "A": {"Ru": 0.6,  "Rv": 0.6,  "path_i": lambda N: round(0.375 * N)},
    "B": {"Ru": 0.5,  "Rv": 0.7,  "path_i": lambda N: round(0.5 * N)},
    "C": {"Ru": 0.4,  "Rv": 0.8,  "path_i": lambda N: N},
}
...
def _genie_decode_interleaved(N, z, b, u_true, v_true, channel):
    ...
    Same algorithm as decoder.decode_single, but feeds TRUE bits (genie).
    """
    n = N.bit_length() - 1
...
def main():
    parser = argparse.ArgumentParser(
        description="MC genie design for polar codes on ABN-MAC")
    parser.add_argument("--class", dest="code_class", required=True,
                        choices=["A", "B", "C"],
                        help="Code class: A=(0.6,0.6), B=(0.5,0.7), C=(0.4,0.8)")
    ...
    log(f"run_design_abnmac.py — MC genie design for ABN-MAC")
    log(f"  Class {args.code_class}: (Ru, Rv) = ({cfg['Ru']}, {cfg['Rv']})")
    log(f"  Channel: I(Z;X)={I_ZX:.4f}, I(Z;Y|X)={I_ZY_X:.4f}, "
        f"I(Z;X,Y)={I_ZXY:.4f}")
