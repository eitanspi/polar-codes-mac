#!/usr/bin/env python3
"""
train_npd_ising_mac.py
======================
Train chained NPD for Ising MAC at N=16, 32 with d=16 h=100.

Ising MAC: 2 Markov states (good/bad).
  Good state: Z = (1-2X) + (1-2Y) + W (normal GMAC)
  Bad state: Z = W (pure noise, no signal)
  State transitions: flip with probability p_flip.

Channel: IsingMAC(sigma2=0.251, p_flip=0.1)
Path: Class C (make_path(N, N))
Design: GMAC_C proxy at SNR=6dB.

This is Priority 1 of the 48-hour session.
"""
...
SIGMA2 = 0.251
P_FLIP = 0.1
SNR_DB = 6  # for design file lookup
D = 16
HIDDEN = 100
N_LAYERS = 2
GRU_LAYERS = 1
...
CONFIGS = {
    16: {'ku': 4, 'kv': 7, 'batch': 32, 'iters_s1': 100_000, 'iters_s2': 50_000},
    32: {'ku': 7, 'kv': 15, 'batch': 16, 'iters_s1': 100_000, 'iters_s2': 50_000},
}
...
def run_one_N(N):
    ...
    print(f'\n{"="*60}\nIsing MAC N={N} d={D} h={HIDDEN} params={model.count_parameters():,}\n{"="*60}')
