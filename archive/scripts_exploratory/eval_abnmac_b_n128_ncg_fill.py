#!/usr/bin/env python3
"""ABNMAC B N=128 NCG fill: only 5/200 in table. Need many more CW."""
...
def main():
    ...
    ckpt_options = [
        'ncg_abnmac_classB_N128_curriculum.pt',
        'ncg_abnmac_classB_N128_best.pt',  # May not exist
    ]
    ckpt = None
    for opt in ckpt_options:
        p = os.path.join(BASE, 'saved_models', opt)
        if os.path.exists(p):
            ckpt = p
            break
    if ckpt is None:
        print("No ABNMAC B N=128 checkpoint found!")
        return
    ...
    model.eval()
    print(f"Loaded: {os.path.basename(ckpt)}", flush=True)
    ...
    out = os.path.join(BASE, 'results', 'abnmac_b_n128_ncg_fill.json')
