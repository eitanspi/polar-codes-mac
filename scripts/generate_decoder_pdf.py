...
def page2_problem(pdf):
    body = (
        ...
        "Instead of learning CalcParent with a naive MLP, we use a gated residual\n"
...
def page10_memory(pdf):
    body = (
        "11. EXTENSION TO CONTINUOUS CHANNELS (GMAC)\n\n"
        "For the Gaussian MAC (Z = (1-2X) + (1-2Y) + W), the discrete\n"
        "embedding is replaced with a simple MLP z_encoder:\n"
        "  Linear(1, 32) -> ELU -> Linear(32, d)\n\n"
        "The tree operations remain identical — only the front-end changes.\n"
        "All tree weights transfer directly from BEMAC training.\n\n"
        "RESULTS (GMAC, SNR=6dB, Class B, rho=0.7):\n"
        "  N=32:  NN BLER=0.040, SC BLER=0.046 (beats SC)\n"
        "  N=64:  NN BLER=0.031, SC BLER=0.025 (1.25x)\n"
        "  N=128: NN BLER=0.032, SC BLER=0.015 (2.11x)\n"
        "  N=256: NN BLER=0.029, SC BLER=0.005 (5.80x)\n\n\n"
        "12. KEY INSIGHTS\n\n"
