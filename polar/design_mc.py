...
def _argsort_with_polar_tiebreak(error_rates):
    """
    Sort channel indices by error rate (ascending), breaking ties using
    the natural polar code reliability ordering.

    For channels with identical MC error rates (common when many channels
    have pe=0 in finite MC trials), ties are broken by the Arikan
    bit-reversal weight: channels whose 0-indexed position has more 1-bits
    in the bit-reversed representation are considered more reliable.

    This prevents the pathological case where np.argsort's unstable
    quicksort arbitrarily orders tied channels, potentially excluding
    critical channels (e.g., the all-pass channel at position N) from
    the info set.

    Parameters
    ----------
    error_rates : np.ndarray, shape (N,)

    Returns
    -------
    sorted_indices : np.ndarray, shape (N,) — 0-indexed channel positions
                     sorted by (error_rate ascending, reliability descending)
    """
    N = len(error_rates)
    n = N.bit_length() - 1

    # Compute bit-reversal weight for each position (0-indexed).
    # In the Arikan polar code, position i has reliability determined by
    # the pattern of + and - splits.  The bit-reversed index's popcount
    # (number of 1-bits) correlates with reliability: higher = more reliable.
    # For exact tiebreaking, we use the full bit-reversed index as a
    # secondary sort key (descending = more reliable first).
    br_weight = np.zeros(N, dtype=np.int32)
    for i in range(N):
        rev = 0
        x = i
        for _ in range(n):
            rev = (rev << 1) | (x & 1)
            x >>= 1
        br_weight[i] = rev  # full bit-reversed index (higher = more reliable)

    # lexsort sorts by last key first, then by earlier keys for ties.
    # We want: primary = error_rates (ascending), secondary = br_weight (descending).
    # lexsort ascending on both, so negate br_weight for descending.
    return np.lexsort((-br_weight, error_rates))
...
def mc_design(n, channel, mc_trials=1000, seed=None, verbose=True,
    ...
    sorted_u = _argsort_with_polar_tiebreak(error_rates_u)
    sorted_v = _argsort_with_polar_tiebreak(error_rates_v)
...
def load_design(filepath):
    ...
    if 'sorted_u' in data:
        ...
        # Use polar tiebreak to ensure critical channels (e.g., position N)
        # are selected when many channels have pe=0.
