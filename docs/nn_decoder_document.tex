...
In this work, we replace the probability tensor manipulations with neural network modules that operate on fixed-dimensional learned embeddings. Each tree operation becomes an MLP forward pass with complexity $O(md)$, where $m$ is the hidden layer width and $d$ is the embedding dimension. The total decoder complexity is $O(md \cdot N \log N)$, independent of the channel model. The Pure Neural CalcParent decoder (Section~\ref{sec:pure_neural}) uses a GRU-gated residual network for \texttt{calcParent}, trained via knowledge distillation. Total: 38,500 parameters.

The decoder is trained with teacher forcing using 4-class cross-entropy loss and employs curriculum learning across code lengths to scale from $N = 8$ to $N = 1024$.
...
\begin{enumerate}
    ...
    \item In the teacher \texttt{calcParent} (used during distillation training), it converts embeddings to log-probabilities for the analytical circular convolution.
\end{enumerate}
...
This module is used in the teacher CalcParent (to re-embed the circular convolution output during distillation) and in the leaf state update (to convert partially deterministic log-probabilities to embeddings).
...
%=============================================================================
\section{Teacher CalcParent (Analytical Baseline)}
\label{sec:soft_bit_bridge}
%=============================================================================

\subsection{Architecture}

The teacher CalcParent is an analytical approach used during knowledge distillation training (Section~\ref{sec:distillation}). It converts embeddings to log-probabilities, applies the exact circular convolution, and converts back. The pipeline at vertex~$\beta$ is:
...
\subsection{Role in Training}

This analytical CalcParent is used exclusively as a \emph{teacher} during knowledge distillation (Section~\ref{sec:distillation}). At inference time, the Pure Neural CalcParent (Section~\ref{sec:pure_neural}) is used instead, eliminating all analytical operations.

The \texttt{Emb2Logits} module is shared with the leaf decision head, which means it is trained with direct supervision (cross-entropy loss at every leaf position). This shared training ensures that the embedding-to-probability mapping is well-calibrated.

\subsection{Differentiability}

The analytical CalcParent is fully differentiable. The circular convolution involves \texttt{logsumexp} operations, which have well-defined gradients:
\begin{equation}
    \frac{\partial}{\partial A_{ij}} \log \sum_k \exp(f_k(A)) = \frac{\sum_k \exp(f_k(A)) \cdot f_k'(A)}{\sum_k \exp(f_k(A))},
\end{equation}
implemented automatically by PyTorch's autograd. This allows gradients to flow through the teacher during distillation without any approximation.

\subsection{Limitation}

The analytical CalcParent retains the $O(|\mathcal{S}|^3)$ complexity of the circular convolution. For memoryless channels with small alphabets (like the BEMAC with $|\mathcal{S}| = 2$), this is negligible. However, for channels with memory where the state space can be large, this becomes the computational bottleneck. This limitation motivates the pure neural \texttt{calcParent} of Section~\ref{sec:pure_neural}.
...
The analytical CalcParent solves the \texttt{calcParent} problem for memoryless channels, but its reliance on the analytical circular convolution means it inherits the $O(|\mathcal{S}|^3)$ complexity for channels with memory. For an ISI channel with memory length $L$, the state space grows as $|\mathcal{S}| = 2^L$, making the convolution $O(8^L)$---exponential in the memory length.
...
Direct end-to-end training of the pure neural \texttt{calcParent} fails: the model converges to a suboptimal local minimum where it ignores the gate and relies entirely on the residual. Knowledge distillation from the analytical CalcParent teacher provides the necessary training signal.
...
\end{equation}
where $K$ is the number of \texttt{calcParent} calls per codeword, $\bm{e}_{\text{student}}$ is the neural output, and $\bm{e}_{\text{teacher}}$ is the analytical CalcParent output (detached from the computation graph). All other parameters (\texttt{calcLeft}, \texttt{calcRight}, \texttt{Emb2Logits}, etc.) are frozen.
...
\subsection{Baseline NCG Decoder}

\begin{table}[H]
\centering
\caption{BLER comparison: baseline NCG decoder vs.\ analytical SC decoder. Class~B path, $R_u \approx 0.5$, $R_v \approx 0.7$.}
\label{tab:bler_sbb}
...
The pure neural version achieves a ratio of 1.30 at $N = 16$---competitive with analytical SC despite having zero analytical operations at inference time. The training loss converges to the same $\sim$0.173 value as the baseline NCG version, indicating that the neural \texttt{calcParent} successfully learns the circular convolution.
...
\textbf{NCG baseline (27K params)} & \textbf{0.059} & \textbf{0.058} & \textbf{1.01} \\
...
\texttt{calcParent} (Baseline) & --- & $O(|\mathcal{S}|^3 + md)$ \\
...
\begin{align}
    ...
    \text{Neural SC (Baseline):} &\quad O\bigl((|\mathcal{S}|^3 + md) \cdot N \log N\bigr), \\
...
\begin{enumerate}
    \item \textbf{Pure Neural CalcParent}: A GRU-gated residual module trained via knowledge distillation that eliminates all analytical operations at inference. At 38,500 parameters and $O(md \cdot N \log N)$ complexity, it is channel-independent and matches or outperforms analytical SC across $N = 8$ to $N = 1024$.
    ...
    \item \textbf{Dual-purpose Emb2Logits}: The shared decision head provides a natural bridge between the latent embedding space and the probability domain, used both for leaf decisions and as the teacher CalcParent input during distillation.
\end{enumerate}
