...
graph framework, using learned MLP operations with a Soft-Bit Bridge
for the calcParent step.
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item \textbf{Neural SC decoder (NCG):}
        MLP-based tree operations with a Soft-Bit Bridge for calcParent,
...
\begin{itemize}
  ...
  \item \textbf{Decision Head (\texttt{emb2logits}):} An MLP
        ...
        $(u, v) \in \{0,1\}^2$. Shared between leaf decisions and the
        Soft-Bit Bridge.

  \item \textbf{Soft-Bit Bridge (calcParent):}
        \begin{enumerate}
          \item Convert child embeddings to log-probabilities via
                \texttt{emb2logits} + temperature-scaled \texttt{log\_softmax}
          \item Reshape to $(2, 2)$ tensors and apply \emph{analytical}
                circular convolution (differentiable via \texttt{torch.logsumexp})
          \item Convert back to embeddings via \texttt{logits2emb} MLP
                ($\mathbb{R}^4 \to \mathbb{R}^d$)
        \end{enumerate}
        This hybrid approach makes calcParent differentiable while leveraging
        the known algebraic structure of the circular convolution.
...
to_git_v2/
  ...
  neural/                       # Neural decoder
    __init__.py
    neural_comp_graph.py        # NCG decoder (MLP + Soft-Bit Bridge)
