...
|----|---------|---------------------|-------------|
| 32 | 0.652   | 0.731               | 10.8%       |
| 64 | 0.442   | 0.575               | 23.1% (training at 4K/40K iterations) |

**Key finding:** The neural decoder learns to exploit channel memory, achieving 10-23% lower BLER than a memoryless SC decoder. The N=64 result was obtained with only 4K out of 40K planned training iterations, suggesting further improvement is possible.

---

## 6. ABNMAC Results (Class C)

Asymmetric Binary Noisy MAC (discrete channel).
...
**Key finding:** NN-SC matches analytical SC on the discrete ABNMAC channel, demonstrating generality beyond Gaussian and erasure channels.
...
Larger model (d=32, 153K parameters) training status at time of writing:

| N  | NN-SC BLER | SC BLER | Ratio | Training progress |
|----|------------|---------|-------|-------------------|
| 32 | 0.0445     | 0.046   | 0.97x (beats SC) | 21K / 62K iterations |

**Key finding:** The larger d=32 model already matches SC at N=32 with only one-third of training complete.
...
| ISI-MAC  | 32   | 0.652        | 0.731        | NN-SC         | NN beats SC    |
| ISI-MAC  | 64   | 0.442        | 0.575        | NN-SC         | NN beats SC    |
